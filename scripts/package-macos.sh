...
# Приложение не заверено у Apple (нотаризация требует платного аккаунта
# разработчика), поэтому при первом запуске macOS покажет «Apple не может
# проверить, что приложение не содержит вредоносного ПО». Кладём инструкцию
# прямо в образ, чтобы человек увидел её ровно в тот момент, когда упрётся.
cat > "$WORK/dist/ЕСЛИ НЕ ОТКРЫВАЕТСЯ.txt" <<'HELP'
Если macOS пишет, что не может проверить приложение —
это нормально, оно просто не заверено у Apple (это платно).

Как разрешить, любой из способов:

СПОСОБ 1 (мышкой)
  1. Перетащи mess в Applications.
  2. Попробуй открыть — появится окно с отказом, нажми "Done".
  3. Системные настройки → Конфиденциальность и безопасность,
     пролистай вниз: там будет строка про mess и кнопка
     "Всё равно открыть" / "Open Anyway". Нажми её.
  4. Открой mess ещё раз и подтверди.

СПОСОБ 2 (одной командой, быстрее)
  Перетащи mess в Applications, открой Терминал и вставь:

      xattr -dr com.apple.quarantine /Applications/mess.app

  После этого приложение открывается обычным двойным щелчком.

Оба способа делаются один раз.
HELP
